...
import { UPWARD_MENU_GAP_PX, upwardMenuMaxHeight } from './upwardMenuSpace'
 ...
 * gapPx 对应弹层 CSS 的 bottom 偏移（标准菜单 6px，@ 提及弹层 8px）。
 */
export const useUpwardMenuClamp = (
  containerRef: RefObject<HTMLElement | null>,
  open: boolean,
  gapPx: number = UPWARD_MENU_GAP_PX,
): void => {
  useEffect(() => {
    ...
    const apply = () => {
      container.style.setProperty(
        ...
        `${upwardMenuMaxHeight(container.getBoundingClientRect().top, gapPx)}px`,
      )
    }
    apply()
